...
extension NutritionService: NutritionServiceProtocol {
    ...
    public func nutrition(type: NutritionType) async throws -> Nutrition {
        let nutritionType = try MBHealthParser.unboxAndCheckIfAvailable(quantityIdentifier: HKQuantityTypeIdentifier(rawValue: type.quantityType.identifier))
        let unitToUse = type.unitMeasure
        let queryDescriptor = HKSampleQueryDescriptor(predicates: [.quantitySample(type: nutritionType)], sortDescriptors: [])
